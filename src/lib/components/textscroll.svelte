...
<script lang="ts">
    ...
    export let text: string = "asdfasdfrwqer";
    ...
    $: isActive = $activeInstance === instanceId && !hideCaretManually;
    ...
    async function animate() {
        ...
        if (typingAudio) {
            const totalAnimationTime = (text.length * typingSpeed) / 1000; // Convert ms to seconds
...
<div class="terminal-text">
    <p class="text-content">
        {displayedText}{#if isActive}<span class="caret" class:visible={isCaretVisible}></span>{/if}
    </p>
</div>
